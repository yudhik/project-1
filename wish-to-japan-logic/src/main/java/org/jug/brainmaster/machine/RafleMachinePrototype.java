...
public class RafleMachinePrototype {
      ...
      for(RegionType regionType : RegionType.values()) {
        ...
        while(winners == null) {
          if(counter < MAX_WINNER_CANDIDATE_COUNT) {
            ...
            System.out.println("No body claim after MAX CANDIDATE COUNTER, put default winner value for region : "+ regionType.getName());
          }
        }
        regionWinner.put(regionType, winners);
        System.out.println("Winner for " + regionType.getName() + " is : "+ regionWinner.get(regionType).getRegistrant().getFirstName());
      }
    }
  ...
  private void getCandidateWinner() {
    ...
    //        for(int i = 1 ; i <=5 ; i++) {
    //      region1WinnerCandidate.add(new GrandPrizeCandidate(new Registrant("voucher" + i, "firstName"+ i, "lastName"+i, "region1mail"+i+"@woo.com", ""+i), RegionType.REGION_1));
    //    }
    ...
    //    for(int i = 1 ; i <=5 ; i++) {
    //      region2WinnerCandidate.add(new GrandPrizeCandidate(new Registrant("voucher" + i, "firstName"+ i, "lastName"+i, "region2mail"+i+"@woo.com", ""+i), RegionType.REGION_2));
    //    }
    ...
    //    for(int i = 1 ; i <=5 ; i++) {
    //      region3WinnerCandidate.add(new GrandPrizeCandidate(new Registrant("voucher" + i, "firstName"+ i, "lastName"+i, "region3mail"+i+"@woo.com", ""+i), RegionType.REGION_3));
    //    }
