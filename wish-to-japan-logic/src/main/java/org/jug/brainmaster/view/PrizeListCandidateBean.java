 ...
 * <p/>
 * This class provides CRUD functionality for all PrizeListCandidate entities.
...
public class PrizeListCandidateBean implements Serializable {
	...
	public void paginate() {
		...
		Root<PrizeListCandidate> root = countCriteria
				.from(PrizeListCandidate.class);
		...
		CriteriaQuery<PrizeListCandidate> criteria = builder
				.createQuery(PrizeListCandidate.class);
		root = criteria.from(PrizeListCandidate.class);
		TypedQuery<PrizeListCandidate> query = this.entityManager
	...
	private Predicate[] getSearchPredicates(Root<PrizeListCandidate> root) {
		...
		if (prizeListWinner != null) {
			predicatesList.add(builder.equal(root.get("prizeListWinner"),
					prizeListWinner));
		}
	...
	/*
	 * Support listing and POSTing back PrizeListCandidate entities (e.g. from
	 * inside an HtmlSelectOneMenu)
	 */

	public List<PrizeListCandidate> getAll() {

		CriteriaQuery<PrizeListCandidate> criteria = this.entityManager
				.getCriteriaBuilder().createQuery(PrizeListCandidate.class);
		return this.entityManager.createQuery(
				criteria.select(criteria.from(PrizeListCandidate.class)))
				.getResultList();
	}
	...
	public Converter getConverter() {
		...
		final PrizeListCandidateBean ejbProxy = this.sessionContext
				.getBusinessObject(PrizeListCandidateBean.class);
